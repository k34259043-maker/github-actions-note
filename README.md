...
2. Actionsから **Note Workflow (Current - Draft Only)** を選び、従来の `theme / target / message / cta / tags` を入力します。
...
`.github/workflows/note.yaml` と `note-perplexity.yaml` は別の旧実験フローで、`is_public=true` の場合は公開操作を含みます。誤選択を避けるためActions上の表示名を `Legacy ... (Can Publish)` としています。通常の実行入口として今回接続しているのは `main.yml` です。旧フローを残すか無効化するかは用途未確認のため、この改修では決めていません。
...
現在の採用版は`generate-v3.txt`、直前の版は`generate-v2.txt`です。サンプル事例は比較方法の入力例であり、検証済みの正解や未使用の最終確認用データではありません。`generation`では、事例ごとに一度収集した資料を共通に使い、旧・新の指示でそれぞれ新規生成します。

`npm test`と`npm run eval:compare`は、Promptfooのテレメトリ・更新確認・共有を無効化する環境フラグを付けて実行します。比較結果はこのリポジトリの実行記録として管理してください。
...
非dry-runでは、生成処理の前に読み取り専用の「自分の記事」画面で認証状態を確認します。保存時は表示中かつ有効な「下書き保存」だけを操作し、自動保存の場合は既定の待機時間を置きます。その後、元の編集画面とは隔離したBrowserContextで同じ編集URLを再読込し、タイトル・本文・記録済みハッシュがすべて一致した場合だけ成功とします。

本文は改行コードと行末の水平空白を正規化してからハッシュ化し、保存処理では確定済み本文を変更しません。ブラウザから読み戻した値は改行コードだけを正規化し、それ以外の相違を成功扱いにしません。Markdownは本文へテキストとして入力するため、リッチテキストとしての見出し・コードの装飾は保証しません。今回の失敗分析と管理された実環境テスト手順は [run 34786753816の調査・修正記録](docs/incidents/2026-09-13-run-34786753816.md) にあります。
