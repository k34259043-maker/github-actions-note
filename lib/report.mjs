...
function summaryRows(manifest, events) {
  ...
  if (note?.type === 'note.failed') noteText = `note処理失敗：${display(note.data.error ?? note.data.reason)}（${note.eventId}）`;
  if (note?.type === 'note.completed') {
    const data = note.data;
    const matches = data.verifiedHash && article?.hash
      ? data.verifiedHash === article.hash ? '保存確認ハッシュは対象原稿と一致' : '保存確認ハッシュが対象原稿と不一致'
      : '保存内容との一致は未確認';
    noteText = `状態：${display(data.status)}。公開：${data.published === true ? '公開済みの記録' : data.published === false ? '未公開の記録' : '未確認'}。${matches}。最終対象の評価：${evaluation ? `${evaluation.id ?? 'ID未取得'}（${display(evaluation.status)}）` : '未取得'}（${note.eventId}）`;
  }
